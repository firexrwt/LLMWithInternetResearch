...
async def list_available_models():
    """
    Получает список доступных моделей с Hugging Face.
    Для каждой модели добавляем флаг installed: True, если локальный файл существует.
    """
    try:
...
async def install_model(model: str):
    """
    Загружает модель, если она ещё не установлена.
    """
    try:
        local_path = model_manager.download_model(model)
        return {"message": f"Модель {model} установлена", "local_path": local_path}
    except Exception as e:
...
async def switch_model(model: str):
    ...
    try:
        ...
        logger.info(f"Загружаем модель: {model}")
        llm_instance = Llama(model_path=model_path, n_ctx=2048, n_threads=8)
        return {"message": f"Модель {model} загружена"}
    except Exception as e:
        logger.error(f"Ошибка загрузки модели: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Ошибка загрузки модели: {str(e)}")

@router.post("/summarize_chat_title")
async def summarize_chat_title(message: str):
    """
    Генерирует краткое название чата (3-4 слова) на основе первого сообщения.
    """
    global llm_instance
    if not llm_instance:
...
@router.post("/query")
async def process_query(text: str, model: str = "Mistral-7B-Instruct", use_internet: bool = False):
    """
    Обрабатывает запрос пользователя через LLaMA.
    Если текущая модель не совпадает с выбранной, происходит переключение.
    """
    global llm_instance
    if not llm_instance:
        raise HTTPException(status_code=500, detail="Модель не загружена")
